function [BARS,LEN,LOC2INT] = GenerateGS...
    (NODE,ELEM,Lvl,RestrictDomain,ColTol,DrawGS)
if nargin<6 || isempty(DrawGS), DrawGS=true; end
if nargin<5 || isempty(ColTol), ColTol=0.9999; end
...
for i=2:Lvl
    ...
    % Collinearity Check
    p = 1; RemoveFlag = zeros(size(I)); Nb = size(BARS,1); % m = 1;
    for j=1:Nn
...
DX = NODE(BARS(:,2),1) - NODE(BARS(:,1),1);
DY = NODE(BARS(:,2),2) - NODE(BARS(:,1),2);
LEN = sqrt(DX.^2 + DY.^2);
NX = DX./LEN; NY = DY./LEN;
LOC2INT = [-NX -NY NX NY];
%% -------------------------------------------------- Draw ground structure
if DrawGS
    figure('Color',[1 1 1]);
    box on; hold on; grid off; axis equal;
    for bar = BARS'
        x = NODE(bar,1); y = NODE(bar,2);
        plot(x,y,'k-','LineWidth',1);
        plot(x,y,'ko','MarkerFaceColor','k','MarkerSize',2);
    end
    title('Initial Ground Structure'); drawnow; hold off;
end
